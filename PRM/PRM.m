...
%PRM 根据地图大小和障碍物生成相应无碰撞的无向有权图的邻接矩阵
    ...
    plot(pointList(:, 1), pointList(:, 2), '.', color='red',markersize=10);
    pause(0);
    for i = 1 : size(adjacentMat, 1)
        for j = 1: size(adjacentMat, 2)
            if adjacentMat(i, j) > 0
                plot([pointList(i, 1), pointList(j, 1)], [pointList(i, 2), pointList(j, 2)], '-', color='#aaaaaa');
            end
        end
    end
    pause(0);
